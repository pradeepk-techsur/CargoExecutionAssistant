...
**Phase 7 note — superseding UI specification:** As of Phase 7, USWDS is being replaced as the visual system, and the replacement has now been decided: the **Carbon Design System** (carbondesignsystem.com), IBM's open-source design system, adopted via the `@carbon/react` npm package. Accordingly, the FR-2.x rules below that name USWDS specifically by component, token, or class (including but not limited to FR-2.1 "USWDS component provenance", FR-2.2 "design tokens", FR-2.3 "asset pipeline", FR-2.5 "USWDS government banner", FR-2.11 "USWDS required indicator", FR-2.12/FR-2.13 "USWDS error wrapper", FR-2.18 "USWDS token pairings", FR-2.20/FR-2.22 component naming, and SM-12 "USWDS conformance") remain **superseded in place**, but now against a named, concrete target rather than an unknown one. In intent: FR-2.1 becomes Carbon component provenance (every interactive control is a `@carbon/react` component, or a documented Carbon-conformant composition); FR-2.2 becomes Carbon design tokens (IBM Design Language type scale, spacing, and colour tokens — e.g. the "White", "Gray 10", "Gray 90", "Gray 100" themes — with no hard-coded hex colour or pixel spacing value); FR-2.3 becomes Carbon's asset pipeline specifically — `@carbon/react`'s underlying Sass package (`@carbon/styles`) compiled to a single static stylesheet loaded via `<link>`, self-hosted IBM Plex as the typeface, and `@carbon/icons-react` SVG icon components — all self-hosted, bundled, and served by the application itself, with no runtime CDN dependency (this constraint does not move because the source system changed, and it is worth stating explicitly here: Carbon's compile-to-static-CSS model is what makes it compatible at all with this project's `style-src 'self'` CSP, which carries no `unsafe-inline` and could not tolerate a runtime CSS-in-JS/styled-components approach); FR-2.5 is **unchanged in requirement** — the official U.S. government banner is a federal statutory element, not a USWDS-specific one, and CargoExec MUST still render it above the header on every screen, built as a Carbon-conformant composition since Carbon has no built-in equivalent component; and FR-2.11 (required-field indication), FR-2.12/FR-2.13 (error-summary and inline-error pattern), FR-2.18 (token-pairing contrast), FR-2.20 (provenance badge), and FR-2.22 (shared state components) each become "the Carbon-conformant equivalent, to the same functional and accessibility standard" rather than a USWDS-named pattern. This document intentionally does **not** rewrite those 26 rules line-by-line or invent specific Carbon component names or props for each one — that is Phase 7 planning's job, done against the actual `@carbon/react` component API. Carbon's own stated compliance target — WCAG AA, Section 508, and EN accessibility standards, per the IBM Accessibility Checklist — is the same bar this project already requires under NFR-2, not a lesser one: adopting Carbon is therefore not an accessibility relaxation, and it does not itself satisfy the per-screen manual review requirement. FR-2.26 is **unchanged**: v1 still adds no automated accessibility gate, and each screen still requires its own signed manual review, including an assistive-technology walkthrough, once rebuilt on Carbon.
