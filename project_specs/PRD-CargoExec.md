...
5. **Meet federal standards as delivered, not as promised.** Section 508 / WCAG 2.1 AA conformance in the shipped UI, achieved by design and review — a hard requirement independent of which visual design system implements it. USWDS was that mechanism through Phase 6; Phase 7 replaces the visual system with the Carbon Design System (`@carbon/react`) while the conformance obligation itself does not move — Carbon's own stated compliance target is WCAG AA / Section 508 / EN standards, the same bar (§5.1 F2).
...
| Layer | Technology |
|-------|------------|
| **Frontend** | React + TypeScript; semantic HTML with full keyboard and assistive-technology support. Through Phase 6, USWDS (U.S. Web Design System) supplied the visual system; **Phase 7 replaces the visual system with the Carbon Design System via `@carbon/react`** while Section 508 / WCAG 2.1 AA conformance remains mandatory and unchanged (§5.1 F2) — Carbon's own stated compliance target (WCAG AA / Section 508 / EN) matches this bar; the per-screen mapping from USWDS primitives to Carbon components is Phase 7 planning's job, not this document |
...
- **Description**: The shared web UI foundation every screen is built on: page layout, banner and header, navigation, form and validation-message patterns, error and empty states, and focus management. This is the feature that makes Section 508 / WCAG 2.1 AA conformance a property of the delivered interface rather than an aspiration — semantic landmarks, an accessible name for every control, visible focus, complete keyboard operability, correct heading order, and screen-reader-announced status and error messaging are established once here and inherited by F6, F8, F10, F12, and F14. Conformance is achieved by design and manual review, including assistive-technology walkthrough of each screen; v1 adds no automated accessibility gate and no CI workflow (§10).
  ...
  **Phase 7 update:** the shared shell is redesigned against the Carbon Design System (carbondesignsystem.com), IBM's open-source design system, adopted via `@carbon/react`, replacing USWDS as the visual system. This preserves the conformance requirement rather than relaxing it: Section 508 / WCAG 2.1 AA conformance stays mandatory and unchanged regardless of which visual system delivers it — Carbon's own stated compliance target (per the IBM Accessibility Checklist) is WCAG AA, Section 508, and EN standards, the same bar. The per-screen mapping from USWDS's interactive primitives to Carbon's component equivalents, and the specific Carbon theme (e.g. White / Gray 10 / Gray 90 / Gray 100), are decisions for Phase 7 planning — not resolved by this PRD update.
- **Capabilities**:
  - Component library, design tokens, typography, and asset pipeline — USWDS-based through Phase 6; **Phase 7 replaces these with the Carbon Design System via `@carbon/react`**, with the exact theme and per-screen component mapping captured during Phase 7 planning
...
| **Content / assets** — visual design system, design tokens, accessible layout patterns | F2 (USWDS through Phase 6; Phase 7 replaces the visual system with Carbon Design System via `@carbon/react`, per-screen mapping deferred to Phase 7 planning) | ✅ feature |
