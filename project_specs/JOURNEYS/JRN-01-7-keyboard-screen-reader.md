...
| Fill the entry form | Tabs field by field through the USWDS entry form; required fields are announced as required from their programmatic marking, and hints are associated with the inputs they describe | Entry form (F6 on F2) | "It tells me a field is required before I fail it, not after." | Efficient | Required-field state carried only by a visual asterisk is silent to her, and she learns the constraint by tripping over it | Programmatic required-field indication and label/hint association, so form structure is announced rather than discovered |
